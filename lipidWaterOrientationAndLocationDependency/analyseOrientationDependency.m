...
if strcmp(trimming,'percentile')
    ...
    trimmingString = sprintf('TRIMMED_%.0f_%.0f',lowerPercentile ...
        ,upperPercentile);
    allR1 = trimmedR1;
elseif strcmp(trimming,'highestValues')
    trimmedR1 = [];
    numberOfAtomsOverValue = [];
    for atomNr = 1:atomCounter
        r1 = allR1(:,:,atomNr);
        if sum(sum(r1 > 20)) < 1
            trimmedR1(:,:,end+1) = r1; %#ok<SAGROW>
        else
            numberOfAtomsOverValue(end+1) = sum(sum(r1>20));
        end
    end
    mean(numberOfAtomsOverValue)
    % just for averaging
    averagedR1Temp = zeros(orientationCount,positionsCount); 
    atomCounterTemp = zeros(orientationCount,positionsCount);
    for orientationNr = 1:orientationCount
        for positionNr = 1:positionsCount
            r1Array = [];
            for atomNr = 1:atomCounter
                r1 = allR1(orientationNr,positionNr,atomNr);
                if r1 <= 20
                    r1Array(end+1) = r1;
                    atomCounterTemp(orientationNr,positionNr) ...
                        = atomCounterTemp(orientationNr,positionNr)+1;
                end
            end
            averagedR1Temp(orientationNr,positionNr) = mean(r1Array);
        end
    end
    effectiveR1 = mean(averagedR1Temp,2)
    overallR1 = mean(effectiveR1)
    allR1 = trimmedR1;
end
