...
samplingFrequencyDecreases = configuration.samplingFrequencyDecreases;
...
logMessage(sprintf(['Found %d hydrogen atoms at %d time ' ...
    ...
    ,path2LogFile,false);
logMessage(sprintf(['Decrease sampling rate for %i times, resulting' ...
    ' in the following times between time steps:'] ...
    ,samplingFrequencyDecreases),path2LogFile,false);
samplingFrequencyArray = [1:samplingFrequencyDecreases]*samplingFrequency;
logMessage(sprintf('%.6d\t',samplingFrequencyArray),path2LogFile,false);
...
r1WithPerturbationTheory = zeros(fibreOrientationsCount ...
    ,positionsInMyelinCount,atomsToCalculate,samplingFrequencyDecreases);
...
for samplingFrequencyNr = 1:length(samplingFrequencyArray)
    configuredLags = ceil(lags/samplingFrequencyNr);
    correlationFunction0W0Saver{samplingFrequencyNr}.('Sec') ...
        = samplingFrequencyArray(samplingFrequencyNr); %#ok<SAGROW>
    correlationFunction0W0Saver{samplingFrequencyNr}.('Values') ...
        = zeros(fibreOrientationsCount ...
        ,positionsInMyelinCount,configuredLags,'like',single(1j)); %#ok<SAGROW>
    correlationFunction1W0Saver{samplingFrequencyNr}.('Sec') ...
        = samplingFrequencyArray(samplingFrequencyNr); %#ok<SAGROW>
    correlationFunction1W0Saver{samplingFrequencyNr}.('Values') ...
        = zeros(fibreOrientationsCount ...
        ,positionsInMyelinCount,configuredLags,'like',single(1j)); %#ok<SAGROW>
    correlationFunction2W0Saver{samplingFrequencyNr}.('Sec') ...
        = samplingFrequencyArray(samplingFrequencyNr); %#ok<SAGROW>
    correlationFunction2W0Saver{samplingFrequencyNr}.('Values') ...
        = zeros(fibreOrientationsCount ...
        ,positionsInMyelinCount,configuredLags,'like',single(1j)); %#ok<SAGROW>
end
...
if configuration.reloadOldSimulation
    ...
    for samplingFrequencyNr = 1:length(samplingFrequencyArray)
        correlationFunction0W0Saver{samplingFrequencyNr}.('Values') ...
            ...
            samplingFrequencyNr}.('Values'); %#ok<SAGROW>
        correlationFunction1W0Saver{samplingFrequencyNr}.('Values') ...
...
for atomNumber = randomSequenceOfAtoms(atomCounter:atomsToCalculate)
    ...
    for positionNumber = 1:positionsInMyelinCount
        ...
        for orientationNumber = 1:fibreOrientationsCount
            ...
            for samplingFrequencyNr = 1:length(samplingFrequencyArray)
                samplingFrequency = samplingFrequencyArray( ...
                    samplingFrequencyNr);
                configuredLags = ceil(lags/samplingFrequencyNr);
                logMessage(sprintf(['Chosen sampling frequency %d sec ' ...
                    'and lags are set to %.1f time steps'] ...
                    ,samplingFrequency,configuredLags),path2LogFile ...
                    ,false)
                ...
                correlationFunction0W0 ...
                    ...
                    1:samplingFrequencyNr:end),configuredLags);
                correlationFunction0W0Saver{samplingFrequencyNr}.Values ...
                    (orientationNumber,positionNumber,:) = (squeeze( ...
                    correlationFunction0W0Saver{samplingFrequencyNr} ...
                    .Values(orientationNumber,positionNumber,:)) ...
                    *(atomCounter-1) + correlationFunction0W0') ...
                    /atomCounter; %#ok<SAGROW>
                ...
                correlationFunction1W0Saver{samplingFrequencyNr}.Values ...
                    (orientationNumber,positionNumber,:) = (squeeze( ...
                    correlationFunction1W0Saver{samplingFrequencyNr} ...
                    .Values(orientationNumber,positionNumber,:)) ...
                    *(atomCounter-1) + correlationFunction1W0') ...
                    /atomCounter; %#ok<SAGROW>
                ...
                correlationFunction2W0Saver{samplingFrequencyNr}.Values ...
                    (orientationNumber,positionNumber,:) = (squeeze( ...
                    correlationFunction2W0Saver{samplingFrequencyNr} ...
                    .Values(orientationNumber,positionNumber,:)) ...
                    *(atomCounter-1) + correlationFunction2W0') ...
                    /atomCounter; %#ok<SAGROW>
                ...
                [spectralDensityW0,spectralDensity2W0] = ...
                    ...
                    ,correlationFunction2W0,omega0,samplingFrequency ...
                    ,configuredLags);
                r1WithPerturbationTheory(orientationNumber,positionNumber ...
                    ,atomCounter,samplingFrequencyNr) = ...
                ...
                scaledR1Rates(orientationNumber,positionNumber ...
                    ...
                    ,positionNumber,atomCounter,samplingFrequencyNr) ...
                    *configuration.scalingRate;
            end
            logMessage(['    Relaxation Rate:' ...
                sprintf(' %.4f',scaledR1Rates( ...
