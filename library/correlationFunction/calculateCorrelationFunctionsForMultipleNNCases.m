function [correlationFunctionsOfCases] = ...
    calculateCorrelationFunctionsForMultipleNNCases( ...
    sphericalHarmonic,numLags,nNCases)
...
correlationFunctions = ifft(multiplicationFunction,[],2,'symmetric');
correlationFunctionsOfCases = zeros(length(nNCases),numLags);

for nNCaseNr = 1:length(nNCases)
    nNCase = nNCases(nNCaseNr);
